...
export default function LoginPage() {
  const { signInWithEmail, signInWithGoogle, user, userProfile, loading, error, clearError } = useAuth();
  ...
  async function handleEmailLogin(e: FormEvent) {
    ...
    try {
      ...
      // Fallback redirect (handled primarily by useEffect guard)
    ...
    <div className="w-full max-w-md my-auto animate-slide-up">
      ...
      <div className="glass-card-dark rounded-2xl p-5 md:p-6 border border-forest-action/30 shadow-lg">
        <div className="text-center mb-4">
          <h1 className="font-display text-2xl md:text-3xl text-ink-inverse mb-1">Welcome back</h1>
          <p className="text-pale-green/80 text-sm">Sign in to track your progress</p>
        </div>
        ...
        {error && (
          <div
            role="alert"
            className="mb-4 p-3 rounded-lg bg-[rgba(217,79,59,0.15)] border border-earth-coral/30 text-earth-coral text-sm"
          >
            {error}
          </div>
        )}
        ...
        <div className="mt-4 text-center">
          <p className="text-sm text-pale-green/80">
            Don&apos;t have an account?{' '}
            <Link href="/signup" className="text-growth-green hover:underline font-medium">
              Create one free
            </Link>
          </p>
        </div>
