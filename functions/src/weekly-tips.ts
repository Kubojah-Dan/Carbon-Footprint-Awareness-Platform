 ...
 */
export const regenerateWeeklyTips = onSchedule(
  {
    ...
    try {
      ...
      for (const userDoc of usersSnap.docs) {
        ...
        try {
          ...
          if (!hasLogs) {
            const baselineResult = calculateBaseline(profile.onboardingAnswers);
          ...
          const previousTipFeedback: Array<{ title: string; feedback: string }> = [];
          ...
          const parsed = JSON.parse(responseText) as { recommendations: any[] };

          const mappedRecommendations: AIRecommendation[] = parsed.recommendations.map((rec, index) => ({
